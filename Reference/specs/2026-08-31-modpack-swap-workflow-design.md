...
| 6 | (2026-09-03) swap 第三参数实现为 `<new_pack>`(新整合包目录,要求含 mods/ 子目录)而非 §2.2 原文的 `<new_mods_dir>`:目录即整合包根语义更自然,且为未来封包/整包(config 等)预留;预检仍只读其 mods/ |
| 7 | (2026-09-03) 冲突备份防覆盖: 重复迁移(--force)时 `_conflict_backup/` 已有同名备份则跳过——首份=目标原始值,不可逆 |
